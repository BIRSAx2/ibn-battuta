...
impl LinKernighan {
    ...
    fn improve_step(&mut self, start: usize) -> bool {
        ...
        while let Some((next, new_gain)) = self.find_next(&t, gain) {
            t.push(next);
            gain = new_gain;
    ...
    fn find_next(&self, t: &[usize], current_gain: f64) -> Option<(usize, f64)> {
    ...
    fn calculate_gain(&self, t: &[usize], next: usize) -> f64 {
        let last = t[t.len() - 1];
        let first = t[0];
    ...
    fn is_tour_feasible(&self, t: &[usize]) -> bool {
    ...
    fn apply_move(&mut self, t: &[usize]) {
